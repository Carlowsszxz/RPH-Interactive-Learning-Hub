        ...
        function getSupabaseClient() {
            ...
            console.error('Supabase client not initialized. Make sure supabase-auth.js is loaded before games.js');
        ...
        const domCache = {
            ...
            triviaRaceCard: document.getElementById('triviaRaceCard'),
            triviaCloseBtn: document.getElementById('triviaCloseBtn'),
            triviaStartBtn: document.getElementById('triviaStartBtn'),
            triviaPlayAgainBtn: document.getElementById('triviaPlayAgainBtn'),
            triviaViewLeaderboardBtn: document.getElementById('triviaViewLeaderboardBtn'),
            triviaBackToEndBtn: document.getElementById('triviaBackToEndBtn'),
            triviaStartScreen: document.getElementById('triviaStartScreen'),
            triviaGameScreen: document.getElementById('triviaGameScreen'),
            triviaEndScreen: document.getElementById('triviaEndScreen'),
            triviaLeaderboardScreen: document.getElementById('triviaLeaderboardScreen'),
            // Sound and Category Elements
            soundToggleBtn: document.getElementById('soundToggleBtn'),
            categoryChips: document.querySelectorAll('.category-chip'),
            filterButtons: document.querySelectorAll('.filter-btn'),
            // Power-up Elements
            powerup5050Btn: document.getElementById('powerup5050'),
            powerupSkipBtn: document.getElementById('powerupSkip'),
            powerupTimeBtn: document.getElementById('powerupTime')
        };

        // Use shorter references
        const { triviaModal, triviaRaceCard, triviaCloseBtn, triviaStartBtn, triviaPlayAgainBtn, triviaViewLeaderboardBtn, triviaBackToEndBtn, triviaStartScreen, triviaGameScreen, triviaEndScreen, triviaLeaderboardScreen } = domCache;
        ...
        if (document.readyState !== 'loading') initLucide(); // If DOM is already loaded
        ...
        triviaModal?.addEventListener('click', (e) => {
            if (e.target === triviaModal) triviaModal.classList.remove('active');
        });
        ...
        domCache.filterButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                domCache.filterButtons.forEach(b => b.classList.remove('active'));
        ...
        function showScreen(screen) {
            const screens = [triviaStartScreen, triviaGameScreen, triviaEndScreen, triviaLeaderboardScreen];
            screens.forEach(el => el?.classList.remove('active'));

            const screenMap = { start: 0, game: 1, end: 2, leaderboard: 3 };
            screens[screenMap[screen]]?.classList.add('active');
            initLucide();
        }
        ...
        // Sound toggle button
        domCache.soundToggleBtn?.addEventListener('click', () => {
            soundEnabled = !soundEnabled;
            domCache.soundToggleBtn?.classList.toggle('active', soundEnabled);
            domCache.soundToggleBtn.innerHTML = soundEnabled
        ...
        domCache.categoryChips.forEach(chip => {
        ...
        // Update topic badges after any category selection change
        domCache.categoryChips.forEach(chip => {
            chip.addEventListener('click', () => {
                setTimeout(updateTopicBadges, 0); // Update after state change
            });
        });
        ...
        domCache.powerup5050Btn?.addEventListener('click', () => {
                ...
                domCache.powerup5050Btn?.classList.add('used');
                domCache.powerup5050Btn.disabled = true;
        ...
        domCache.powerupSkipBtn?.addEventListener('click', () => {
                if (powerups.skip) return;
                
                playSound('powerup');
                powerups.skip = true;
                domCache.powerupSkipBtn?.classList.add('used');
                domCache.powerupSkipBtn.disabled = true;
        ...
        domCache.powerupTimeBtn?.addEventListener('click', () => {
                if (powerups.extraTime) return;
                
                playSound('powerup');
                powerups.extraTime = true;
                domCache.powerupTimeBtn?.classList.add('used');
                domCache.powerupTimeBtn.disabled = true;
        ...
        function startTriviaGame() {
            ...
            if (domCache.powerup5050Btn) { domCache.powerup5050Btn.classList.remove('used'); domCache.powerup5050Btn.disabled = false; }
            if (domCache.powerupSkipBtn) { domCache.powerupSkipBtn.classList.remove('used'); domCache.powerupSkipBtn.disabled = false; }
            if (domCache.powerupTimeBtn) { domCache.powerupTimeBtn.classList.remove('used'); domCache.powerupTimeBtn.disabled = false; }
        ...
        async function saveScore(score, correctAnswers, totalQuestions, accuracy, timeTaken, bestStreak, performanceGrade) {
            try {
                ...
                // Update game_leaderboard with complete statistics
                try {
                    // Fetch current leaderboard entry to calculate new average
                    const { data: existingEntry } = await supabaseClient
                        .from('game_leaderboard')
                        .select('total_plays,average_score,best_score')
                        .eq('user_id', userId)
                        .eq('game_type', 'trivia')
                        .maybeSingle();
                    ...
                    if (existingEntry) {
                        newTotalPlays = (existingEntry.total_plays || 0) + 1;
                        const prevSum = (existingEntry.average_score || 0) * (existingEntry.total_plays || 1);
                        newAverageScore = Math.round((prevSum + score) / newTotalPlays * 100) / 100;
                        newBestScore = Math.max(existingEntry.best_score || 0, score);
                    }
                    ...
                    const { data: leaderboardData, error: leaderboardError } = await supabaseClient
                        ...
                        .upsert([{
                            user_id: userId,
                            game_type: 'trivia',
                            game_name: 'Historical Trivia Race',
                            score: score,
                    ...
                    if (leaderboardError) {
                        console.warn('Warning: Could not update leaderboard, but score was saved:', leaderboardError);
                    } else {
                        console.log('Leaderboard updated successfully:', leaderboardData);
                    }
                } catch (lbException) {
                    console.warn('Exception updating leaderboard entry:', lbException);
                }
        ...
        // Timeline Quest Modal Elements (cached in domCache)
        domCache.timelineQuestCard = document.getElementById('timelineQuestCard');
        domCache.timelineModal = document.getElementById('timelineModal');
        domCache.timelineCloseBtn = document.getElementById('timelineCloseBtn');
        domCache.timelineStartBtn = document.getElementById('timelineStartBtn');
        domCache.timelineSoundToggleBtn = document.getElementById('timelineSoundToggleBtn');
        domCache.timelinePlayAgainBtn = document.getElementById('timelinePlayAgainBtn');
        domCache.timelineViewLeaderboardBtn = document.getElementById('timelineViewLeaderboardBtn');
        domCache.timelineBackToEndBtn = document.getElementById('timelineBackToEndBtn');
        domCache.timelineStartScreen = document.getElementById('timelineStartScreen');
        domCache.timelineGameScreen = document.getElementById('timelineGameScreen');
        domCache.timelineEndScreen = document.getElementById('timelineEndScreen');
        domCache.timelineLeaderboardScreen = document.getElementById('timelineLeaderboardScreen');
        domCache.eraChips = document.querySelectorAll('.era-chip');

        // Shorter references
        const { timelineQuestCard, timelineModal, timelineCloseBtn, timelineStartBtn, timelineSoundToggleBtn, timelinePlayAgainBtn, timelineViewLeaderboardBtn, timelineBackToEndBtn, timelineStartScreen, timelineGameScreen, timelineEndScreen, timelineLeaderboardScreen, eraChips } = domCache;
        ...
        // Leaderboard Filter Buttons
        const timelineLeaderboardFilters = document.querySelectorAll('.timeline-leaderboard-filters .filter-btn');
        timelineLeaderboardFilters.forEach(btn => {
            btn.addEventListener('click', function() {
                timelineLeaderboardFilters.forEach(b => b.classList.remove('active'));
                this.classList.add('active');
                loadTimelineLeaderboard(this.dataset.filter);
            });
        });
